...
st.set_page_config(
    ...
    page_icon="images/heart_logo.jpg",
    layout="wide",
...
def main():
    ...
    if st.button("Show Me!!"):
        ...
        if output == 0:
            recipe = return_recipe("data/recipes.csv", output, food_types)
            ...
            with st.beta_expander("Did you cook this recipe?", expanded=True):
                ...
                with c2:
                    st.image("images/instagram.jpg", width=60)

            st.subheader("More Recommendations")
            recipes3 = return_recipe3("data/recipes.csv", output, food_types)
            ...
            return_recipe("data/recipes.csv", output, food_types)
            st.success('{}'.format(recipe.to_string(
                columns=['name'], header=False, index=False).upper()))
            st.subheader("Nutritional Value")
            st.table(recipe[['total fat (PDV)', 'sugar (PDV)', 'sodium (PDV)',
